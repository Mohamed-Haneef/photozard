...
class UserSession
{
    ...
    public static function authenticate($user, $pass, $fingerprint=null)
    {
        ...
        if ($username['status_code'] == 200) {
            ...
            if ($conn->query($sql)) {
                ...
                return ['status' => 'success', 'response' => 'Login success, Authenticated', 'status_code' => 200];
            } else {
                return ['status' => 'error', 'response' => 'Login details not updated', 'status_code' => 300];
            }
        } else {
            return ['status' => 'error', 'response' => $username['response']. "Response: $user", 'status_code' => $username['status_code']];
        }
    }
    ...
    public function isValid()
    {
        ...
        if (isset($this->data['login_time'])) {
            ...
            if (10000 > time() - $login_time->getTimestamp()) {
                return true;
            } else {
                return false;
            }
